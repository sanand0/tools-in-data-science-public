(function () {
  ...
  const getRawUrl = () => {
    ...
    return `https://raw.githubusercontent.com/sanand0/tools-in-data-science-public/main/${p.endsWith(".md") || !p ? p || "README" : p + ".md"}`;
  };
  ...
  const showMsg = (msg, err) => { status.textContent = msg; status.style.color = err ? "red" : "var(--ai-p)"; setTimeout(() => status.textContent = "", 2000); };
  const setScope = (s) => { curScope = s; btnPage && btnPage.classList.toggle("active", s === "page"); btnSel && btnSel.classList.toggle("active", s === "selection"); };
  const onOpen = () => { panel.classList.add("open"); curSel = getSel() || curSel; btnSel.disabled = !curSel; setScope(curSel ? "selection" : "page"); textarea.focus(); };
  const sync = () => { panel && panel.classList.remove("open"); curSel = ""; setScope("page"); };
  const createUI = () => {
    if (document.querySelector(".ai-l")) return;
    const style = document.createElement("style"); style.textContent = CSS; document.head.appendChild(style);
    const launcher = document.createElement("button"); launcher.className = "ai-l"; launcher.innerHTML = "✨ Ask AI"; launcher.onclick = onOpen;
    ...
    PRESETS.forEach(p => {
      ...
      b.onclick = () => { textarea.value = p.question; showMsg("Preset added"); }; presetsDiv.appendChild(b);
    });
    panel.querySelector(".ai-close").onclick = () => panel.classList.remove("open");
    btnPage.onclick = () => setScope("page"); btnSel.onclick = () => curSel ? setScope("selection") : showMsg("No text selected", true);
    ...
    panel.querySelector(".ai-btn-sub").onclick = () => {
      let p = getActivePrompt(); if (p) {
        window.open((PROVIDERS.find(x => x.id === select.value) || PROVIDERS[0]).url + encodeURIComponent(p), "_blank", "noopener,noreferrer");
  ...
  const handleInline = (el) => {
    if (el.hasAttribute("preset")) {
      ...
      PRESETS.forEach(p => {
        ...
        b.onclick = () => { createUI(); textarea.value = p.question; onOpen(); setScope("page"); }; span.appendChild(b);
      });
      el.replaceWith(span);
    } else {
      ...
      btn.onclick = () => { createUI(); textarea.value = q; onOpen(); setScope("page"); }; el.replaceWith(btn);
    }
  };
